...
class Solution {
    ...
    void solve() {
        while (io.hasNext()) {
            int n = io.nextInt();
            int[] w = new int[5];
            for (int i = 0; i < n; i++) w[i] = io.nextInt();
            if (n <= 3) {
                io.println(1024);
            } else if (n == 4) {
                int ans = 0;
                for (int i = 0; i <= 2012; i++) {
                    w[4] = i;
                    ans = Math.max(ans, getFive(w, 5));
                }
                io.println(ans);
            } else {
                io.println(getFive(w, n));
            }
        }
    }

    int getFive(int[] w, int n) {
        int ans = 0;
            ...
            for (int j = i+1; j < n; j++) {
                for (int k = j+1; k < n; k++) {
                    if ((w[i]+w[j]+w[k]) % 1024 == 0) {
                        ans = Math.max(ans, (sum-w[i]-w[j]-w[k]-1) % 1024 +1);
                    }
                }
            }
        }
